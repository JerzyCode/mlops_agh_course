...
def save_mcp_image(content: str, tool_name: str):
    ...
    try:
        ...
        if not image_data.startswith(b"\x89PNG"):
            print(f"DEBUG: Data starts with:\n\n {content}\n\n")
            return f"Error: Tool {tool_name} returned text instead of PNG. Check server logs."
...
async def make_llm_request(prompt: str) -> str:
    mcp_servers = {
        ...
        "visualization-server": "http://localhost:8003/mcp",
    }
        ...
        for _ in range(10):
            ...
            for tool_call in response.tool_calls:
                ...
                if func_name == "line_plot":
                    final_content = save_mcp_image(str(func_result), func_name)
                else:
                    final_content = str(func_result)
...
if __name__ == "__main__":
    ...
    prompt = "Display a linear function plot for y = 2x for x in range -5 to 5. As tittle use current date. Do not use CODE."
    response = asyncio.run(make_llm_request(prompt))
    print("Response:\n", response)
